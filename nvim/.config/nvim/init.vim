...
func! Debug()
	if expand( '%:e' ) == 'cpp'
		exec 'terminal g++ %<.cpp -o %<.run -g -DDEBUG ' . g:compile_options . '&& gdb ./%<.run'
	elseif expand( '%:e' ) == 'py'
		exec 'terminal python -m pdb ./%'
	endif
endfunc

func! Compile()
	if expand( '%:e' ) == 'html'
		exec 'terminal firefox %:p'
	elseif expand( '%:e' ) == 'tex'
		exec 'terminal (tectonic % ) && echo "50\%..." && (tectonic %>/dev/null 2>&1) && echo "100\%...Done" && ( rm -f %<.aux %<.log %<.toc %<.out %<.nav %<.snm) && firefox %<.pdf'
	elseif expand( '%:e' ) == 'md'
		exec 'terminal md2pdf.sh %<.md %<.pdf && firefox %<.pdf'
	elseif expand( '%:e' ) == 'rs'
	...
	elseif expand( '%:e' ) == 'r'
		exec "terminal R %"
	elseif expand( '%:e' ) == 'cu'
		exec 'terminal nvcc %<.cpp -o %<.run && time ./%.run'
	else
		exec 'terminal g++ %<.cpp -o %<.run -O2 ' . g:compile_options . ' && time ASAN_OPTIONS=detect_leaks=0 ./%<.run'
	endif
endfunc
